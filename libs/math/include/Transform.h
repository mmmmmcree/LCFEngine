...
namespace lcf {
    ...
    public:
        ...
        void setLocalMatrix(Matrix4x4<float> matrix) noexcept;
        ...
        void setWorldMatrix(Matrix4x4<float> world_matrix) noexcept;
        const Matrix4x4<float> & getWorldMatrix() const noexcept;
        const Matrix4x4<float> & getLocalMatrix() const noexcept { return m_local_matrix; }
    ...
    private:
        ...
        mutable Matrix4x4<float> m_world_matrix;
    };
    
    class CachedInvertedMatrix
    {
    public:
        CachedInvertedMatrix(const Matrix4x4<float> & original_matrix) : m_original_matrix_p(&original_matrix) {}
        void markDirty() noexcept { m_is_dirty = true; }
        void cleanDirty() noexcept { m_is_dirty = false; }
        bool isDirty() const noexcept { return m_is_dirty; }
    ...
    private:
        const Matrix4x4<float> * m_original_matrix_p = nullptr;
        mutable bool m_is_dirty = true;
        mutable Matrix4x4<float> m_matrix;
    };
