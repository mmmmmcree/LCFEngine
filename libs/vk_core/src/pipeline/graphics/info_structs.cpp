...
const AttachmentDescriptionInfo & AttachmentSetInfo::at(ColorAttachmentKey key) const noexcept
{
    assert(key.validate(m_set_id) and "key from a different attachment set");
    return m_descriptions[key.getIndex()];
}

AttachmentDescriptionInfo & AttachmentSetInfo::at(ColorAttachmentKey key) noexcept
{
    assert(key.validate(m_set_id) and "key from a different attachment set");
    return m_descriptions[key.getIndex()];
}

const AttachmentDescriptionInfo & AttachmentSetInfo::at(ResolveAttachmentKey key) const noexcept
{
    assert(key.validate(m_set_id) and "key from a different attachment set");
    return m_descriptions[m_color_resolve_list[key.getIndex()].second];
}

AttachmentDescriptionInfo & AttachmentSetInfo::at(ResolveAttachmentKey key) noexcept
{
    assert(key.validate(m_set_id) and "key from a different attachment set");
    return m_descriptions[m_color_resolve_list[key.getIndex()].second];
}

const AttachmentDescriptionInfo & AttachmentSetInfo::at(DepthStencilAttachmentKey key) const noexcept
{
    assert(key.validate(m_set_id) and "key from a different attachment set");
    return m_descriptions.back();
}

AttachmentDescriptionInfo & AttachmentSetInfo::at(DepthStencilAttachmentKey key) noexcept
{
    assert(key.validate(m_set_id) and "key from a different attachment set");
    return m_descriptions.back();
}
