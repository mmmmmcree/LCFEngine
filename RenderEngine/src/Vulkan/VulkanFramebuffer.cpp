...
void lcf::render::VulkanFramebuffer::beginRendering()
{
    ...
    for (uint32_t i = 0; i < m_color_attachments.size(); ++i) {
        ...
        color_image->transitLayout(vk::ImageLayout::eColorAttachmentOptimal);
        color_attachment_infos[i].setImageLayout(color_image->getLayout())
    ...
    if (m_depth_stencil_attachment) {
        ...
        depth_stencil_image->transitLayout(vk::ImageLayout::eDepthStencilAttachmentOptimal);
        depth_attachment_info.setImageView(depth_image_view)
            .setImageLayout(depth_stencil_image->getLayout())
            .setLoadOp(vk::AttachmentLoadOp::eClear)
            .setStoreOp(vk::AttachmentStoreOp::eStore)
    ...
    if (m_msaa_resolve_attachment) {
        ...
        msaa_resolve_image->transitLayout(vk::ImageLayout::eColorAttachmentOptimal);
        color_attachment_infos[src_index].setResolveMode(resolve_mode)
            .setResolveImageLayout(msaa_resolve_image->getLayout())
