...
A feature is declared as a single pointer-to-member NTTP — `conf::k_feature<&vk::PhysicalDeviceVulkan12Features::timelineSemaphore>` means "this member must be true"; the variable template derives both the enable side (set true) and the test side (read from the queried chain) via `lcf::member_pointer_traits`, so modules never write per-feature lambdas.

All content is constexpr. `core_since` drops the extension names when the chosen API version includes the capability as core. `conf::FeatureChain` assembles a pNext chain **at runtime** (`std::unordered_map<type_index, std::any>` — node-based for pNext address stability; the head `PhysicalDeviceFeatures2` is seeded in the constructor) from whatever feature structs the selected dependencies reference — an open set, so it cannot be a fixed `vk::StructureChain` (C++ cannot enumerate all feature structs without reflection or codegen). Type safety is kept via the `conf::feature_struct_c` concept: `request`/`get` constrain the struct to `vk::StructExtends<T, vk::PhysicalDeviceFeatures2>::value`, whose per-struct specializations are generator-produced and grow with each Vulkan release — so `k_feature<&T::member>` accepts any real (present or future) feature struct and rejects anything else, with no central hand-written list. Exemplars: `sync/feature_dependencies.h` (core feature, no extensions) and `surface/feature_dependencies.h` (`VK_KHR_surface` + `VK_KHR_swapchain`, optional `k_swapchain_maintenance_dependency`; platform window-system instance extensions come from the GUI side with `SurfaceProvider`, not from the manifest).
...
- 2026-06-10 aaaa573: FeatureChain stays dynamic (open set); feature_struct_c via vk::StructExtends gives type safety + auto-extension
- 2026-06-10 aaaa573: (reverted) fixed superset vk::StructureChain — cannot enumerate all features without reflection/codegen
