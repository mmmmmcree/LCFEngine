...
namespace lcf {
    ...
    public:
        ...
        bool empty() const noexcept { return m_last == m_first; }
        ...
        void shrink_to_fit();
        void clear_and_shrink() noexcept { this->discard_block(); }
        [[nodiscard]] std::expected<void, std::error_code> try_reserve(std::size_t n) noexcept { return n <= this->capacity() ? std::expected<void, std::error_code>{} : this->tryReallocateTo(n); }
        ...
        template <typename... Args>
        [[nodiscard]] std::expected<std::reference_wrapper<T>, std::error_code> try_emplace_back(Args &&... args) noexcept(std::is_nothrow_constructible_v<T, Args &&...>);
        ...
        template <std::ranges::input_range R>
        void assign_range(R && r) { this->clear(); this->append_range(std::forward<R>(r)); }
        template <std::ranges::input_range R>
        void append_range(R && r);
        template <std::ranges::input_range R>
        iterator insert_range(const_iterator pos, R && r);
        std::span<const std::byte> counted_bytes() const noexcept { return this->compute_byte_span(this->size()); }
        std::span<const std::byte> counted_bytes_with_capacity() const noexcept { return this->compute_byte_span(this->capacity()); }
        ...
        std::span<T> data_span() noexcept { return {m_first, this->size()}; }
        std::span<const T> data_span() const noexcept { return {m_first, this->size()}; }
...
namespace lcf {
    ...
    FlexArray_Opus_4_7<T, SizeType, Allocator>::FlexArray_Opus_4_7(std::from_range_t, R && r, const Allocator & alloc) : m_allocator(alloc)
    {
        if constexpr (std::ranges::sized_range<R>) {
            ...
            for (auto && element : r) { this->emplace_back(std::forward<decltype(element)>(element)); }
        }
    }
    ...
    auto FlexArray_Opus_4_7<T, SizeType, Allocator>::operator=(const FlexArray_Opus_4_7 & other) -> FlexArray_Opus_4_7 &
    {
        ...
        if constexpr (std::allocator_traits<Allocator>::propagate_on_container_copy_assignment::value) {
            if (m_allocator != other.m_allocator) { this->clear_and_shrink(); }
        }
    ...
    template <typename T, std::unsigned_integral SizeType, typename Allocator>
    void FlexArray_Opus_4_7<T, SizeType, Allocator>::shrink_to_fit()
    {
        const std::size_t current_size = this->size();
        if (current_size == this->capacity()) { return; }
        if (current_size == 0) { this->clear_and_shrink(); return; }
    ...
    template <typename T, std::unsigned_integral SizeType, typename Allocator>
    template <typename... Args>
    [[gnu::always_inline]] inline T & FlexArray_Opus_4_7<T, SizeType, Allocator>::emplace_back(Args &&... args)
    {
        if (m_last == m_end) [[unlikely]] { this->ensure_one_more_capacity(); }
        T * slot_p = m_last;
    ...
    template <typename T, std::unsigned_integral SizeType, typename Allocator>
    template <typename... Args>
    auto FlexArray_Opus_4_7<T, SizeType, Allocator>::try_emplace_back(Args &&... args) noexcept(std::is_nothrow_constructible_v<T, Args &&...>)
    ...
    template <typename T, std::unsigned_integral SizeType, typename Allocator>
    void FlexArray_Opus_4_7<T, SizeType, Allocator>::pop_back() noexcept
    {
    ...
    template <typename T, std::unsigned_integral SizeType, typename Allocator>
    template <std::ranges::input_range R>
    void FlexArray_Opus_4_7<T, SizeType, Allocator>::append_range(R && r)
    {
        if constexpr (std::ranges::sized_range<R>) {
    ...
    template <typename T, std::unsigned_integral SizeType, typename Allocator>
    template <std::ranges::input_range R>
    auto FlexArray_Opus_4_7<T, SizeType, Allocator>::insert_range(const_iterator pos, R && r) -> iterator
    {
