...
namespace {
    ...
    {
        lcf::FlexArray<int> a(0, 42);
        EXPECT_EQ(a.size(), 0u);
        EXPECT_EQ(a.capacity(), 0u);
        lcf::FlexArray<int> b(static_cast<std::size_t>(0));
        EXPECT_EQ(b.size(), 0u);
    ...
    TEST(FlexArrayConstruction, EmptyIteratorRange)
    {
        ...
        lcf::FlexArray<int> a(empty.begin(), empty.end());
        EXPECT_EQ(a.size(), 0u);
    }
    ...
    TEST(FlexArrayConstruction, SingleElementRange)
    {
        ...
        lcf::FlexArray<int> a(one.begin(), one.end());
        EXPECT_EQ(a.size(), 1u);
        EXPECT_EQ(a[0], 7);
    }
    ...
    {
        lcf::FlexArray<int> a;
        lcf::FlexArray<int> b = std::move(a);
        EXPECT_EQ(b.size(), 0u);
        EXPECT_EQ(b.capacity(), 0u);
    }
    ...
    TEST(FlexArrayConstruction, CrossAllocatorMoveDoesElementWiseMove)
    {
        ...
        lcf::FlexArray<std::string, std::uint32_t, PA> b(std::move(a), alloc2);
        EXPECT_EQ(b.size(), 2u);
    ...
    TEST(FlexArrayConstruction, FromRangeWithUnsizedFilterView)
    {
        ...
        lcf::FlexArray<int> a(std::from_range, evens);
        EXPECT_EQ(a.size(), 3u);
        EXPECT_EQ(a[0], 2);
        EXPECT_EQ(a[2], 6);
    }
